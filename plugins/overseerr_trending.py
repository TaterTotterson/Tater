...
from helpers import format_irc, redis_client, get_tater_name
...
class OverseerrTrendingPlugin(ToolPlugin):
    ...
    required_settings = {
        ...
        "DEFAULT_PAGE_SIZE": {
            "label": "Default Page Size (display count)",
            "type": "number",
            "default": 10,
        },
        "DEFAULT_PAGE": {
            "label": "Default API Page (1 = first)",
            "type": "number",
            "default": 1,
        },
    }
    ...
    def _get_settings():
        ...
        page_size = int(s.get("DEFAULT_PAGE_SIZE", 10) or 10)
        default_page = int(s.get("DEFAULT_PAGE", 1) or 1)
        return base, api, page_size, default_page

    def _fetch_list(self, kind: str, when: str, page: int):
        base, api_key, _, _ = self._get_settings()
        if not api_key:
    ...
    def _fmt_date(d):
        if not d:
            return ""
        try:
            # NOTE: %-d is POSIX; if on Windows, replace with %#d
            return datetime.strptime(d, "%Y-%m-%d").strftime("%b %-d, %Y")
        except Exception:
            return d

    def _format_items_text(self, items, kind_label: str, limit: int):
        lines = []
        for i, it in enumerate(items[:limit], start=1):
            ...
            # --- Build final display (no stars, no TMDB, no full release date) ---
            line = f"{i}. {title}"
            if year:
                line += f" ({year})"
            if short_blurb:
    ...
    def _build_answer(self, args):
        ...
        Args (from LLM):
          kind: 'movies' or 'tv'
          when: 'trending' or 'upcoming'
        Paging is internal and comes from settings.DEFAULT_PAGE.
        """
        kind = (args.get("kind") or "").lower().strip()
        when = (args.get("when") or "").lower().strip()
        _, _, default_limit, default_page = self._get_settings()
        limit = int(default_limit or 10)
        page = int(default_page or 1)  # fixed internally
        ...
        data = self._fetch_list(kind, when, page)
        if "error" in data:
            return data["error"]
    ...
    async def handle_homeassistant(self, args, llm_client):
        """
        TTS-friendly for Home Assistant:
          - Speaks header + up to 5 numbered entries
          - Each entry: "Number <n>. <Title> <Year>. <Overview>"
          - Parentheses removed from year; no stars/TMDB/full date
        """
        ...
        # Match lines like: "1. Title (2025)" or "2. Show (2022)"
        ...
        # Limit to 5 items for voice and prefix with "Number <n>."
