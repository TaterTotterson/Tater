  ...
  <PopupTransition :open="open" backdrop-class="tv-modal-backdrop tr-backdrop" @close="closePopup">
      <section class="tv-modal tr-modal" role="dialog" aria-modal="true" aria-label="Runtime statistics">
        ...
        <div v-else class="tr-grid">
          ...
          <article class="tv-panel tr-card calls"><header><div><span class="tv-eyebrow">Language models</span><h2>LLM Calls</h2><p>{{ num(llm.running_total ?? llm.active_total) }} running • {{ num(llm.queued_total) }} queued • Started {{ num(llm.totals?.started) }} • Completed {{ num(llm.totals?.completed) }} • Failed {{ num(llm.totals?.failed) }}</p></div></header><section class="tr-block"><h3>Current Calls</h3><div v-if="llmCalls.length" class="tr-list"><article v-for="(call, index) in llmCalls" :key="call.id || index"><div><strong>{{ call.source_label || call.label || 'Unknown source' }}</strong><small>{{ callMeta(call, 'llm') }}</small></div><span class="tv-state" :class="{ good: text(call.state) !== 'queued' }">{{ call.state_label || (text(call.state) === 'queued' ? 'Queued' : 'Running') }} {{ age(call.state_age_seconds ?? call.age_seconds) }}</span></article></div><div v-else class="tv-empty compact">No active LLM calls right now.</div></section></article>
