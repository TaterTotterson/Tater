...
const modelSummary = computed(() => {
  ...
  return [
    ...
    num(modelsPayload.value.managed_loaded_count) ? `${num(modelsPayload.value.managed_loaded_count)} managed voice` : "",
...
function modelMeta(model: JsonRow): string {
  return [text(model.kind_label || model.category), text(model.provider_label || model.provider || "Local"), text(model.device) ? `Device ${text(model.device)}` : "", num(model.estimated_bytes) ? `${text(model.memory_kind || "ram").toUpperCase()} est ${bytes(model.estimated_bytes)}` : "Estimate unavailable", loadedAt(model.loaded_ts)].filter(Boolean).join(" • ");
}
