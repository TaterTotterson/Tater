...
class CoreMusicLibraryRendererTests(unittest.TestCase):
    ...
    def test_music_library_uses_component_frontend_and_event_stream(self) -> None:
        ...
        self.assertIn('grid-template-columns: 82px minmax(190px, 1fr)', music_styles)
        ...
        self.assertIn('.tm-player > .tm-queue {', music_styles)
        ...
        self.assertIn("nudgePlayer(row.target, -10)", music_player)
        self.assertIn("nudgePlayer(row.target, 10)", music_player)
        self.assertIn("offsetLabel(playerSetting(row.target).sync_offset_ms)", music_player)
        ...
        self.assertIn('.tm-sync-nudges {', music_styles)
