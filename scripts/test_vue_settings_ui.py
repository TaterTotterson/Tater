...
class VueSettingsTests(unittest.TestCase):
    ...
    def test_voice_workspace_and_all_subtabs_are_vue_owned(self) -> None:
        ...
        self.assertIn("Tater Presence Intelligence", presence)
        self.assertIn("Room-by-room location", presence)
        self.assertIn("Stable identity registry", presence)
        self.assertIn("Identity Resolving Key", presence)
        self.assertIn("Satellite RSSI offsets", presence)
        self.assertIn("without Home Assistant", presence)
        self.assertIn("new EventSource", presence)
        self.assertIn("presence.snapshot", presence)
        self.assertIn("presence.event", presence)
        self.assertIn("tvoice-presence-rooms", presence)
        self.assertIn("compareByDistance", presence)
        self.assertIn("nearest first", presence)
        self.assertIn("compareRegistryDevices", presence)
        self.assertIn("registeredDevices", presence)
        self.assertIn("Add a nearby device", presence)
        self.assertIn("not live distance order", presence)
        self.assertIn("ROOM_PREVIEW_LIMIT = 6", presence)
        self.assertIn("postJson", presence)
        self.assertIn("save_scanner", presence)
        self.assertIn('class="tv-modal tvoice-presence-room-modal"', presence)
        self.assertIn("View all {{ group.devices.length }}", presence)
        self.assertNotIn('<TransitionGroup name="presence-device"', presence)
        self.assertIn(".tvoice-presence-room-modal-body", styles)
        self.assertIn(".tpresence-discovered", styles)
        self.assertIn("endpointWithDefaults(props.snapshotEndpoint)", presence)
        ...
        self.assertIn('@router.get("/api/tater/satellite/v1/presence/config")', voice_routes)
        self.assertIn('@router.post("/api/tater/satellite/v1/presence/config")', voice_routes)
        self.assertIn('@router.get("/api/tater/satellite/v1/presence/history")', voice_routes)
