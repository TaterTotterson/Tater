...
def write_hydra_ledger(
    *,
    ...
    entry = {
        ...
        "assistant_response": short_text_fn(assistant_response, limit=900),
        ...
        "thanatos_ms": max(0, int(thanatos_ms or 0)),
        "progress_ms": max(0, int(progress_ms or 0)),
        "state_update_ms": max(0, int(state_update_ms or 0)),
        "state_update_deterministic_count": max(0, int(state_update_deterministic_count or 0)),
        "state_update_llm_count": max(0, int(state_update_llm_count or 0)),
    ...
    hermes_llm_text = short_text_fn(hermes_llm, limit=180)
    if hermes_llm_text:
