...
def astraeus_system_prompt(*, platform: str) -> str:
    ...
    return (
        ...
        "- Never output extraction/parsing/resolve-only steps without a concrete executable tool action.\n"
        "- When a downstream step needs IDs/links/URIs from earlier step outputs, plan the downstream tool step directly; Thanatos can read prior tool-result payloads to pull exact values.\n"
        "- If the requested outcome cannot be executed with currently available tools, return steps=[].\n"
