...
def astraeus_system_prompt(*, platform: str) -> str:
    return (
        ...
        "- For Discord by-name routing, prefer targets.guild_name plus targets.channel (for example #tater); use channel_id only when the user provides an ID.\n"
        "- When a file/image should be sent, plan send_message with attachments using available artifact_id values.\n"
...
def thanatos_system_prompt(
    *,
    ...
    return (
        ...
        "- For Discord destination by name, pass targets.guild_name and targets.channel instead of guessing ids.\n"
        "- For send-with-file requests, include attachments with exact artifact_id values in send_message arguments.\n"
