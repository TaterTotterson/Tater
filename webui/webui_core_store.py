...
def _update_core_entry(entry: dict) -> tuple[bool, str]:
...
def _update_core_entries(entries: list[dict]) -> tuple[list[str], list[str]]:
    updated: list[str] = []
    failed: list[str] = []

    for entry in entries:
        ok, msg = _update_core_entry(entry)
        if ok:
...
def _render_updates_tab(catalog_items: list[dict], catalog_errors: list[str]):
    ...
    if action_col.button(
        ...
        with st.spinner(f"Updating {len(update_entries)} core(s)..."):
            updated, failed = _update_core_entries(update_entries)
        flash_messages = []
        if updated:
