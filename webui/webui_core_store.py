...
from webui.webui_cores import render_core_controls
...
CORE_MANAGER_FLASH_KEY = "core_manager_flash_messages"
...
def _queue_core_manager_messages(messages: list[dict]) -> None:
    normalized = []
    ...
    if normalized:
        st.session_state[CORE_MANAGER_FLASH_KEY] = normalized


def _render_core_manager_messages() -> None:
    messages = st.session_state.pop(CORE_MANAGER_FLASH_KEY, None)
...
def _render_catalog_warnings(catalog_errors: list[str]) -> None:
    if not catalog_errors:
        return
    st.warning("Some core repos could not be loaded.")
    for error in catalog_errors:
        st.caption(error)


def _restart_core_runtime(
    entry: dict,
    *,
    start_core_fn=None,
    stop_core_fn=None,
    ...
    if not module_key:
        return False, "module key is missing; restart Tater to load the new core code"
    if not callable(start_core_fn) or not callable(stop_core_fn):
        return False, "automatic runtime restart is unavailable; restart Tater to load the new core code"

    try:
        stop_core_fn(module_key)
        start_core_fn(module_key)
        return True, "runtime restarted"
    except Exception as exc:
        return False, f"hot-restart failed ({exc}); restart Tater to load the new core code"


def _update_core_entry(
    entry: dict,
    *,
    start_core_fn=None,
    stop_core_fn=None,
    ...
    if not platform_id:
        return False, "Core id is missing."
    if not isinstance(catalog_item, dict):
        return False, f"{platform_id}: no catalog update source is available."

    ok, msg = install_core_from_shop_item(catalog_item)
    if not ok:
        return False, msg
    ...
    restarted, restart_msg = _restart_core_runtime(
        entry,
...
def _render_core_store_tab(catalog_items: list[dict], catalog_errors: list[str], manifest_repos: list[dict[str, str]]):
    _render_catalog_warnings(catalog_errors)

    available_items = [
        item for item in catalog_items
        if not is_core_installed(str(item.get("id") or "").strip())
    ]

    search_q = st.text_input(
        "Search available cores",
        value="",
        placeholder="Search name, id, description...",
        key="core_manager_store_search",
    ).strip().lower()

    filtered_items = []
    for item in available_items:
        platform_id = str(item.get("id") or "").strip()
        name = str(item.get("name") or _core_display_name(platform_id)).strip()
    ...
    st.caption(
        f"Showing {len(filtered_items)} of {len(available_items)} available core(s) across {len(manifest_repos)} repo(s)."
    )

    if not filtered_items:
        st.info("No downloadable cores match the current filters.")
        return
    ...
    for row_start in range(0, len(filtered_items), 2):
        ...
        for col_idx, item in enumerate(row_items):
            platform_id = str(item.get("id") or "").strip()
            display_name = str(item.get("name") or _core_display_name(platform_id)).strip()
            description = str(item.get("description") or "").strip()
            source_label = str(item.get("_source_label") or "Custom Repo").strip()
            module_key = str(item.get("module_key") or _core_module_key(platform_id)).strip()
            ...
            with row_cols[col_idx]:
                with st.container(border=True):
                    ...
                    if st.button(
                        "Install",
                        key=f"core_manager_install_{platform_id}",
                        type="primary",
                        use_container_width=True,
                    ):
                        ok, msg = install_core_from_shop_item(item)
                        if ok:
                            _queue_core_manager_messages([{"level": "success", "text": msg}])
                            st.rerun()
                        st.error(msg)


def _render_installed_cores_tab(
    ...
    *,
    ...
    core_by_key = {
        ...
        for item in (core_registry or [])
        if isinstance(item, dict) and str(item.get("key") or "").strip()
    }

    search_q = st.text_input(
        "Search installed cores",
        value="",
        placeholder="Search name, id, description...",
        key="core_manager_installed_search",
    ).strip().lower()
    ...
    st.caption(f"Showing {len(filtered_entries)} of {len(installed_entries)} installed core(s).")
    if not filtered_entries:
        st.info("No installed cores match the current filters.")
        return

    for entry in filtered_entries:
        platform_id = entry["id"]
        display_name = entry["display_name"]
        purge_key = f"core_manager_purge_{platform_id}"
        ...
        with st.container(border=True):
            ...
            core_settings = core_by_key.get(str(entry.get("module_key") or "").strip())
            if core_settings:
                ...
                st.caption("Settings are unavailable because this core is not currently importable.")

            controls = st.columns([1, 1, 3])
            if entry["update_available"]:
                if controls[0].button(
                    "Update",
                    key=f"core_manager_update_{platform_id}",
                    type="primary",
                    use_container_width=True,
                ):
                    ok, msg = _update_core_entry(
                ...
                controls[0].button(
                    "Up to date",
                    disabled=True,
                    key=f"core_manager_uptodate_{platform_id}",
                    use_container_width=True,
                )
            ...
            if controls[1].button(
                "Remove",
                key=f"core_manager_remove_{platform_id}",
                type="secondary",
                use_container_width=True,
            ):
                ok, msg = uninstall_core_file(platform_id)
                if not ok:
                    st.error(msg)
                    continue
                ...
                if purge_redis:
                    ok2, msg2 = clear_core_redis_data(platform_id, module_key=entry["module_key"])
                    flash_messages.append({"level": "success" if ok2 else "error", "text": f"Redis cleanup: {msg2}"})
                _queue_core_manager_messages(flash_messages)
                st.rerun()
...
def _render_updates_tab(
    ...
    *,
    ...
    if action_col.button(
        "Update All",
        key="core_manager_update_all",
        disabled=not update_entries,
        type="primary",
        use_container_width=True,
    ):
        with st.spinner(f"Updating {len(update_entries)} core(s)..."):
            updated, failed = _update_core_entries(
                update_entries,
                start_core_fn=start_core_fn,
                stop_core_fn=stop_core_fn,
            )
        ...
        if updated:
            ...
            flash_messages.append({"level": "success", "text": f"Updated {len(updated)} core(s): {updated_text}"})
        if failed:
            failed_text = "; ".join(failed[:4])
            if len(failed) > 4:
                failed_text += f"; and {len(failed) - 4} more"
            flash_messages.append({"level": "error", "text": f"{len(failed)} core update(s) failed: {failed_text}"})
        if not flash_messages:
            flash_messages.append({"level": "info", "text": "No core updates were available."})
        _queue_core_manager_messages(flash_messages)
        st.rerun()

    if update_entries:
        text_col.caption("Only cores with a newer catalog version are listed below. Update one at a time or update all in one pass.")
    else:
        text_col.caption("No catalog-backed core updates are currently available.")

    if not update_entries:
        if local_only_entries:
            st.info("All catalog-backed cores are up to date. Some installed cores are local-only and do not have a catalog update source.")
        else:
            st.success("All installed catalog-backed cores are up to date.")
        return
    ...
    search_q = st.text_input(
        ...
        value="",
        placeholder="Search name, id, description...",
        key="core_manager_updates_search",
    ).strip().lower()
    ...
    st.caption(f"Showing {len(filtered_entries)} of {len(update_entries)} core(s) with available updates.")
    if not filtered_entries:
        st.info("No core updates match the current filters.")
        return
    ...
    for entry in filtered_entries:
        ...
        with st.container(border=True):
            ...
            if st.button(
                "Update",
                key=f"core_manager_updates_update_{platform_id}",
                type="primary",
                use_container_width=True,
            ):
                ok, msg = _update_core_entry(
...
def _render_settings_tab(catalog_errors: list[str], manifest_repos: list[dict[str, str]]):
    st.caption("The default Core Shop is always enabled. Add optional names for extra repos to control the source label shown in the store.")

    default_name_col, default_url_col = st.columns([1, 2])
    with default_name_col:
        st.text_input(
            "Default repo name",
            value=DEFAULT_CORE_SHOP_LABEL,
            disabled=True,
            key="core_manager_default_repo_name",
        )
    with default_url_col:
        st.text_input(
            "Default manifest URL",
            value=CORE_SHOP_MANIFEST_URL_DEFAULT,
            disabled=True,
            key="core_manager_default_manifest_url",
        )

    extra_repos = get_additional_core_shop_manifest_repos()
    if "core_manager_repo_form_count" not in st.session_state:
        st.session_state["core_manager_repo_form_count"] = max(1, len(extra_repos))
        for idx, repo in enumerate(extra_repos):
            st.session_state[f"core_manager_repo_name_{idx}"] = repo.get("name", "")
            st.session_state[f"core_manager_repo_url_{idx}"] = repo.get("url", "")

    st.caption("Additional repos")
    for idx in range(max(1, int(st.session_state.get("core_manager_repo_form_count", 1)))):
        name_key = f"core_manager_repo_name_{idx}"
        url_key = f"core_manager_repo_url_{idx}"
        cols = st.columns([1, 2])
        with cols[0]:
            st.text_input(
                f"Repo {idx + 1} name",
        ...
        with cols[1]:
            st.text_input(
                ...
                key=url_key,
                placeholder="https://example.com/core_manifest.json",
            )

    add_col, remove_col, save_col, refresh_col = st.columns([1, 1, 1, 1])

    if add_col.button("Add Repo", key="core_manager_add_repo", type="secondary", use_container_width=True):
        next_idx = int(st.session_state.get("core_manager_repo_form_count", 1))
        st.session_state["core_manager_repo_form_count"] = next_idx + 1
        st.session_state[f"core_manager_repo_name_{next_idx}"] = ""
        st.session_state[f"core_manager_repo_url_{next_idx}"] = ""
        st.rerun()

    if remove_col.button("Remove Last", key="core_manager_remove_repo", type="secondary", use_container_width=True):
        count = max(1, int(st.session_state.get("core_manager_repo_form_count", 1)))
        if count > 1:
            last_idx = count - 1
            st.session_state.pop(f"core_manager_repo_name_{last_idx}", None)
            st.session_state.pop(f"core_manager_repo_url_{last_idx}", None)
            st.session_state["core_manager_repo_form_count"] = count - 1
        else:
            st.session_state["core_manager_repo_name_0"] = ""
            st.session_state["core_manager_repo_url_0"] = ""
        st.rerun()

    if save_col.button("Save Repos", key="core_manager_save_repos", type="primary", use_container_width=True):
        parsed_repos = []
        row_count = max(1, int(st.session_state.get("core_manager_repo_form_count", 1)))
        for idx in range(row_count):
            name = _normalize_manifest_name(st.session_state.get(f"core_manager_repo_name_{idx}"))
            url = _normalize_manifest_url(st.session_state.get(f"core_manager_repo_url_{idx}"))
        ...
        save_additional_core_shop_manifest_repos(parsed_repos)
        st.success("Core repos saved.")
        st.rerun()

    if refresh_col.button("Refresh Catalog", key="core_manager_refresh_catalog", type="secondary", use_container_width=True):
        st.rerun()

    st.caption(
        "Catalog merge order is fixed: the default Core Shop loads first, then your additional repos. "
        "If two repos publish the same core id, the first one wins."
    )
...
def render_core_store_page(
    *,
    ...
    _render_core_manager_messages()

    manifest_repos = get_configured_core_shop_manifest_repos()
    catalog_items, catalog_errors = load_core_shop_catalog(manifest_repos)

    installed_tab, store_tab, updates_tab, settings_tab = st.tabs(
        ["Installed Cores", "Core Store", "Updates", "Settings"]
    )

    with installed_tab:
        _render_installed_cores_tab(
            catalog_items,
            catalog_errors,
    ...
    with store_tab:
        _render_core_store_tab(catalog_items, catalog_errors, manifest_repos)
