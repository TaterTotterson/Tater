...
def _apply_native_ota_update_locked(
    ...
    *,
    ...
    if progress is not None:
        # Reserve 100% for the post-reboot version check. A device can finish
        # writing an image and still roll back to its previous partition.
    ...
    if isinstance(client_status, dict):
        ...
        if not connected:
            ...
            if reconnected:
                session["ota_reconnect_seen"] = True
                ...
                if actual_version:
                    _fail_native_ota_locked(
                        session,
                        f"The satellite reconnected on {actual_version}, but Tater expected {expected_version or 'the new firmware'}. The update may have rolled back or been interrupted.",
                    )
                    return
    ...
    if bool(session.get("ota_reboot_requested")) or bool(session.get("ota_disconnect_seen")):
        session["message"] = "Firmware downloaded. Waiting for the satellite to reconnect on the new version."
                ...
                if isinstance(session, dict):
                    ...
                    if native_ota:
                        _apply_native_ota_update_locked(
                            ...
                            [],
                            native_client,
                            status_error=native_status_error or session["device_log_error"],
                        )
        else:
            with _FIRMWARE_SESSION_LOCK:
                session = _FIRMWARE_SESSIONS.get(session_id)
