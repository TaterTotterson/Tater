...
def settings_snapshot(selector: Any = "") -> Dict[str, Any]:
    token = _selector_token(selector)
    global_raw = _raw_settings()
    if not token:
        return normalize_settings(global_raw)
    return normalize_settings(_raw_settings(token), base=normalize_settings(global_raw))


def firmware_settings_snapshot(selector: Any = "") -> Dict[str, Any]:
    current = settings_snapshot(selector)
...
def settings_fields(selector: Any = "") -> List[Dict[str, Any]]:
    current = settings_snapshot(selector)
    return [
        {
            "key": "wake_section",
...
def save_settings(values: Dict[str, Any], *, selector: Any = "") -> Dict[str, Any]:
    token = _selector_token(selector)
    global_raw = _raw_settings()
    device_raw = _raw_settings(token) if token else {}
    current_raw = {**global_raw, **device_raw}
    current = settings_snapshot(token)
    profile = _profile_for_save(values or {}, current_raw)
    normalized = normalize_settings(values or {}, base={**current_raw, **profile})
