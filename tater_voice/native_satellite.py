...
def _live_settings_payload(selector: str = "") -> Dict[str, Any]:
    from . import native_live_settings

    return native_live_settings.settings_snapshot(selector)


def _firmware_settings_payload(selector: str = "") -> Dict[str, Any]:
    from . import native_live_settings

    return native_live_settings.firmware_settings_snapshot(selector)
...
def _client_snapshot(selector: str, row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "live_settings": _live_settings_payload(selector),
        "auth": {
...
async def live_settings(selector: str = "") -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "selector": token,
        "settings": native_live_settings.settings_snapshot(token),
        "fields": native_live_settings.settings_fields(token),
    }
...
async def push_live_settings(selector: str = "") -> Dict[str, Any]:
    token = _text(selector)
    settings = _live_settings_payload(token)
    firmware_settings = _firmware_settings_payload(token)
    pushed: list[str] = []
    async with _clients_lock:
        ...
        for target_selector, row in targets.items():
            ...
            if not isinstance(queue, asyncio.Queue):
                continue
            queued = _envelope("settings", _firmware_settings_payload(str(target_selector)))
...
async def save_live_settings(values: Dict[str, Any], *, selector: str = "") -> Dict[str, Any]:
    ...
    result = native_live_settings.save_settings(values or {}, selector=selector)
...
async def handle_websocket(websocket: WebSocket) -> None:
    ...
    try:
        ...
        await send_json(_envelope("settings", _firmware_settings_payload(selector)))
